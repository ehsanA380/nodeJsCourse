  ...
  return (
    <>   
      ...
      {directoryItems.map(({name,isDirectory}, i,) => (
        <div key={i} id={i}>
          {name}
           {!(isDirectory) && <a href={`${URL}files/${name}?action=open`}>Open</a>}{" "}
           {(isDirectory) && <a href={`./${name}`}>Open</a>}{" "}


          {!(isDirectory)&&<a href={`${URL}files/${name}?action=download`}>Download</a>}
          <button onClick={()=>{inputField(name,i)}}>rename</button> 
          {/* <button onClick={()=>{inputField(item,i)}}>input</button>  */}
          <button id="renameBtn" onClick={()=>{handleDelete(name)}} >delete</button>
